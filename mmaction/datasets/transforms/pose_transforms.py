...
class JointToBone(BaseTransform):
    ...
    Args:
        dataset (str): Define the type of dataset: 'nturgb+d', 'openpose',
            'coco', 'custom'. Defaults to ``'nturgb+d'``.
    ...
    def __init__(self,
        ...
        self.target = target
        if self.dataset not in ['nturgb+d', 'openpose', 'coco', 'custom']:
            raise ValueError(
                f'The dataset type {self.dataset} is not supported')

        # Define bone pairs for the custom dataset, customize this based on your keypoints
        if self.dataset == 'custom':
            # Custom dataset with 16 keypoints, define your pairs here
            self.pairs = [
                (0, 1),  # example: nose -> head_top
                (1, 2),  # head_top -> spine
                (2, 3),  # spine -> left shoulder
                (3, 4),  # left shoulder -> left elbow
                (4, 5),  # left elbow -> left hand
                (2, 6),  # spine -> right shoulder
                (6, 7),  # right shoulder -> right elbow
                (7, 8),  # right elbow -> right hand
                (2, 9),  # spine -> pelvis
                (9, 10), # pelvis -> left knee
                (10, 11),# left knee -> left foot
                (9, 12), # pelvis -> right knee
                (12, 13),# right knee -> right foot
            ]
        elif self.dataset == 'nturgb+d':
