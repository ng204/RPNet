...
def parse_args():
    ...
    parser.add_argument('out_dir', help='output directory for processed videos')
    parser.add_argument(
        '--config',
        default=('/root/mmaction2/configs/skeleton/stgcnpp/stgcnpp_8xb16-joint-u100-80e_ntu60-xsub-keypoint-2d.py'),
        help='skeleton model config file path')
    parser.add_argument(
        '--checkpoint',
        default=('/root/mmaction2/work_dirs/stgcnpp_8xb16-joint-u100-80e_ntu60-xsub-keypoint-2d_16epoch/best_acc_top1_epoch_13.pth'),
        help='skeleton model checkpoint file/url')
    parser.add_argument(
        '--det-config',
        default='/root/mmdetection/configs/rtmdet/rtmdet_l_8xb32-300e_coco.py',
        help='human detection config file path (from mmdet)')
    parser.add_argument(
        '--det-checkpoint',
        default=('/root/mmdetection/work_dirs/rtmdet_l_8xb32-300e_coco/epoch_120.pth'),
    ...
    parser.add_argument(
        '--pose-config',
        default='/root/mmaction2/demo/td-hm_hrnet-w32_8xb64-210e_coco-256x192.py',
        help='human pose estimation config file path (from mmpose)')
    parser.add_argument(
        '--pose-checkpoint',
        default=('/root/mmpose/work_dirs/td-hm_hrnet-w32_8xb64-210e_coco-256x192/best_coco_AP_epoch_190.pth'),
        help='human pose estimation checkpoint file/url')
    parser.add_argument(
        '--label-map',
        default='/root/mmaction2/tools/data/skeleton/label_map_h.txt',
        help='label map file')
    parser.add_argument(
...
def batch_process_videos(args):
    video_files = [f for f in os.listdir(args.video_dir) if f.endswith('.mp4')]
    
    for video_file in video_files:
        video_path = os.path.join(args.video_dir, video_file)
        output_path = os.path.join(args.out_dir, f'output_{video_file}')
        print(f"Processing {video_file}...")
        
        # Set the video file path and output file path dynamically
        args.video = video_path
        args.out_filename = output_path
...
def process_single_video(args):
    ...
    if not frames:
        print(f"Error: No frames extracted from video {args.video}. Skipping this video.")
        return
...
def visualize(args, frames, data_samples, action_label):
    ...
    vid.write_videofile(args.out_filename, codec='libx264', remove_temp=True)
